use binuid_shared_wasm::toml::{Table, Value};
...
pub(crate) fn get_dependency_path(name: &str, version: Option<&str>, path: Option<&str>) -> String {
    match (path, version) {
        (Some(p), Some(ver)) => {
...
pub(crate) fn get_dependency_zip_path(name: &str, version: Option<&str>, path: Option<&str>) -> String {
