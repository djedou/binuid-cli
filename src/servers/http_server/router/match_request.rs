...
macro_rules! match_request {
    ($request_method:expr, $request_path:expr, {
        ...
        let result = $crate::_match_request_regex!(
            ...
            $request_path,
            {
                $($crate::servers::router::match_request::pattern($pattern) => {
                    $($method => $result),*
                }),*
            }
        );
        result.map(move |(value, names, captures)| {
            (value, $crate::servers::router::match_request::Params::from_captures(
                names,
                captures
            ))
        })
...
macro_rules! _match_request_regex {
    ($request_method:expr, $request_path:expr, {
        ...
        use $crate::servers::router::match_request::Error;
        use binuid_shared::{
            regex::Regex,
            hyper::Method,
